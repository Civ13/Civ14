...
public sealed class WeatherNomadsSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var nomads))
        {
            ...
            {
                continue; // Precipitation not ready
            }
    ...
    private void UpdateTileWeathers(EntityUid uid, WeatherNomadsComponent nomads)
    {
        ...
        foreach (var tile in gridAtmosphere.Tiles.Values)
        {
            ...
            {
                ApplyWeatherToTile(gridUid.Value, tileRef, weatherType, grid, gridAtmosphere, roofComp);
            }
            else
            {
    ...
    /// </summary>
    private void ApplyWeatherToTile(EntityUid gridUid, TileRef tileRef, string weatherType, MapGridComponent grid,
        GridAtmosphereComponent gridAtmosphere, RoofComponent? roofComp)
    {
        ...
        // Apply weather visuals globally (consider tile-specific visuals if needed)
